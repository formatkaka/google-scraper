...
func buildGoogleUrls(searchTerm string, langCode string, countryCode string, pages int, count int) ([]string, error) {
	toScrape := []string{}
	searchTerm = strings.Trim(searchTerm, " ")
	searchTerm = strings.Replace(searchTerm, " ", "+", -1)

	if baseUrl, found := googleDomains[countryCode]; found {
		for i := 0; i < pages; i++ {
			start := i * count
			scrapeURL := fmt.Sprintf("%s%s&num=%d&hl=%s&start=%d&filter=0", baseUrl, searchTerm, count, start, langCode)
...
func scrapeClientRequest(searchURL string, proxyString interface{}) (*http.Response, error) {
	...
	if res.StatusCode != 200 {
		err := fmt.Errorf("scraper received non 200 status code")
		return nil, err
	}
...
func GoogleScrape(query string, langCode string, countryCode string, proxyString interface{}, pages int, count int, timeout int) ([]SearchResult, error) {
	results := []SearchResult{}
	resultCounter := 0
	googlePages, err := buildGoogleUrls(query, langCode, countryCode, pages, count)
...
func main() {
	...
	res, err := GoogleScrape("alia bhatt", "en", "com", nil, 1, 10, 10)
